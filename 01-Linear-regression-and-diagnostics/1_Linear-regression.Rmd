...
The relationship above appears linear, but we can do better than eyeballing. Let's test that using a simple linear regression between the two variables. 

Remember that basic linear regression seeks to XXXXXXX R's standard linear regression function is lm(). Calls take the form y ~ x, with an optional second argument specifying the data frame that R should look in to find the model variables.
